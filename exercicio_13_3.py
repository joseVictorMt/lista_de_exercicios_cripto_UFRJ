...
#Exercicio 13.3 - Teste de Lucas Melhorado

from math import sqrt

def fatoracaoIngenua(x):
    fat_list = [] #Lista de fatores.
...
def testeDeLucasMelhorado(n):
    ...
    for base in range(2, n-1):
        ...
        for fator in fatores:
            currExpo = expo/fator #Expoente atual
            if currExpo not in expoList:
                print currExpo
                if(potenciacaoModular(base, currExpo, n) != 1):
                    expoList.append(currExpo);
                else:
                    achou = True;
